...
const CreatePost = () => {
  ...
  const generateImage = async () => {
    if (form.prompt) {
      try {
        ...
        const response = await fetch("http://localhost:8088/api/v1/dalle", {
          method: "POST",
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        const response = await fetch("http://localhost:8088/api/v1/post", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
    ...
    <section className="max-w-7xl mx-auto">
      <div>
        ...
        <p className="mt-2 text-[#666e75] text-[16px] max-w-[750px]">
          Create imaginative as visually stunning images through DALL-E AI and
